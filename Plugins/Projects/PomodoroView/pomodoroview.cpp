...
void PomodoroView::onAllReferencesSet()
{
    for(auto iter = m_referencesMap.begin(); iter != m_referencesMap.end(); ++iter)
    {
        auto&& interfaceName = iter.key();
        auto&& plugin = iter.value();
        if(!QString::compare(interfaceName, "IPomodoroModel", Qt::CaseInsensitive))
        {
            myModel = castPluginToInterface<IPomodoroModel>(plugin);
...
void PomodoroView::onAllReferencesReady()
{
...
void PomodoroView::on_buttonExit_clicked()
{
    emit onClose(this);
}
