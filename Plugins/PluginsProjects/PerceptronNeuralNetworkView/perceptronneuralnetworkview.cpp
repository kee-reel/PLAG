...
void PerceptronNetworkViewPlugin::onAllReferencesSet()
{
    for(auto iter = m_referencesMap.begin(); iter != m_referencesMap.end(); ++iter)
    {
        auto&& interfaceName = iter.key();
        auto&& plugin = iter.value();
        if(!QString::compare(interfaceName, "INeuralNetworkModel", Qt::CaseInsensitive))
        {
            model = castPluginToInterface<INeuralNetworkModel>(plugin);
        }
    }
    PluginBase::onAllReferencesSet();
}

void PerceptronNetworkViewPlugin::onAllReferencesReady()
{
    isStatsChanged = true;
...
void PerceptronNetworkViewPlugin::MarkNetworkStatsToUpdate()
{
    isStatsChanged = true;
    ui->buttonResumeTraining->setEnabled(false);
}

bool PerceptronNetworkViewPlugin::UpdateNetworkStats()
{
...
void PerceptronNetworkViewPlugin::UpdateLayerStatsGUI()
{
...
void PerceptronNetworkViewPlugin::MakePlot(int graph, QVector<double> &x, QVector<double> &y)
{
    ui->customPlot->graph(graph)->setData(x, y);
}

void PerceptronNetworkViewPlugin::ReplotPlot()
{
...
QJsonObject PerceptronNetworkViewPlugin::ConvertNetworkParams()
{
...
QJsonObject PerceptronNetworkViewPlugin::ConvertLayerParams(LayerType type, LayerParams &params)
{
    QString typeStr;
    switch (type) {
    case Input:
...
void PerceptronNetworkViewPlugin::on_buttonRunTrain_clicked()
{
...
void PerceptronNetworkViewPlugin::on_buttonResumeTraining_clicked()
{
    ui->checkTrain->setChecked(false);
    ui->checkTest->setChecked(false);
...
void PerceptronNetworkViewPlugin::RunTrainFromEpoch(int maxEpoch)
{
...
void PerceptronNetworkViewPlugin::on_buttonClose_clicked()
{
    emit onClose(this);
}

void PerceptronNetworkViewPlugin::on_buttonAdd_clicked()
{
...
void PerceptronNetworkViewPlugin::on_buttonRemove_clicked()
{
    QModelIndex index = ui->listView->selectionModel()->currentIndex();
    if(index.isValid())
    {
...
void PerceptronNetworkViewPlugin::on_listView_clicked(const QModelIndex &index)
{
    UpdateLayerStatsGUI();
}

void PerceptronNetworkViewPlugin::on_spinSize_valueChanged(int arg1)
{
    QModelIndex index = ui->listView->selectionModel()->currentIndex();
    if(!index.isValid())
        return;
...
void PerceptronNetworkViewPlugin::on_spinLearnSpeed_valueChanged(double arg1)
{
    QModelIndex index = ui->listView->selectionModel()->currentIndex();
    if(!index.isValid())
        return;
...
void PerceptronNetworkViewPlugin::on_spinMoment_valueChanged(double arg1)
{
    QModelIndex index = ui->listView->selectionModel()->currentIndex();
    if(!index.isValid())
        return;
...
void PerceptronNetworkViewPlugin::on_spinFuncIndent_valueChanged(double arg1)
{
    QModelIndex index = ui->listView->selectionModel()->currentIndex();
    if(!index.isValid())
        return;
...
void PerceptronNetworkViewPlugin::on_spinBias_valueChanged(double arg1)
{
    QModelIndex index = ui->listView->selectionModel()->currentIndex();
    if(!index.isValid())
        return;
...
void PerceptronNetworkViewPlugin::on_buttonLoadImage_clicked()
{
    QFileDialog dialog(
                this,
                "Add Image",
...
void PerceptronNetworkViewPlugin::on_buttonRemoveImages_clicked()
{
    trainImages.clear();
    trainImagesModel.clear();
}

void PerceptronNetworkViewPlugin::on_buttonLoadImage_2_clicked()
{
    QFileDialog dialog(
                this,
                "Add Image",
...
void PerceptronNetworkViewPlugin::on_buttonRemoveImages_2_clicked()
{
    testImages.clear();
    testImagesModel.clear();
}

void PerceptronNetworkViewPlugin::on_spinEpoch_editingFinished()
{
    MarkNetworkStatsToUpdate();
}

void PerceptronNetworkViewPlugin::on_spinErrorThreshold_editingFinished()
{
    MarkNetworkStatsToUpdate();
}

void PerceptronNetworkViewPlugin::on_spinMinWeight_editingFinished()
{
    MarkNetworkStatsToUpdate();
}

void PerceptronNetworkViewPlugin::on_spinMaxWeight_editingFinished()
{
    MarkNetworkStatsToUpdate();
}

void PerceptronNetworkViewPlugin::on_spinTestErrorThreshold_editingFinished()
{
    MarkNetworkStatsToUpdate();
}
