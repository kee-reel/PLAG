...
void HeartRateDeviceModel::onAllReferencesSet()
{
    for(auto iter = m_referencesMap.begin(); iter != m_referencesMap.end(); ++iter)
    {
        ...
        if(!QString::compare(interfaceName, "IExperimentControlModel", Qt::CaseInsensitive))
        {
            ...
            deviceManager = qobject_cast<IExperimentControlModel*>(instance);
        }
        else if(!QString::compare(interfaceName, "IFuzzyLogicModel", Qt::CaseInsensitive))
        {
            auto instance = plugin->getObject();
            fuzzyLogic = qobject_cast<IFuzzyLogicModel*>(instance);
        }
    }
    PluginBase::onAllReferencesSet();
}

void HeartRateDeviceModel::onAllReferencesReady()
{
    savedBeatsCount = 10;
...
void HeartRateDeviceModel::CompareValues(int index)
{
...
void HeartRateDeviceModel::ChooseDataIndex(int index)
{
    heartRateDataIndex = index;
}

void HeartRateDeviceModel::StartTracking()
{
    heartData = deviceManager->GetRegistersLineSeries();
    HRData = heartData.at(heartRateDataIndex);
    connect(HRData, &QLineSeries::pointAdded, this, &HeartRateDeviceModel::CompareValues);
}

void HeartRateDeviceModel::StopTracking()
{
    disconnect(HRData, &QLineSeries::pointAdded, this, &HeartRateDeviceModel::CompareValues);
}
