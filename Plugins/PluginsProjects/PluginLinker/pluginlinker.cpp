...
QSharedPointer<LinkerItem> PluginLinker::createLinkerItem(QWeakPointer<IPluginHandler> pluginHandler)
{
    ...
    m_linkerItemsMap.insertMulti(linkerItemPtr->getInterfaceName(), linkerItemPtr);
    ++m_pluginUidCounter;
...
QWeakPointer<LinkerItem> PluginLinker::getCorePlugin()
{
    return m_corePlugin;
}

QMap<QString, QWeakPointer<LinkerItem> > &PluginLinker::getClildPluginsMap()
{
    ...
    linkerItemsMapCopy.remove(m_corePlugin->getInterfaceName());
